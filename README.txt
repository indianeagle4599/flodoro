...
* **Timer controls** – Start/Pause, Reset and Postpone Break controls.  Circular **+** and **−** buttons now sit directly above and below the timer so you can add or subtract a minute without searching for them.  Keyboard shortcuts mirror these functions (Space toggles play/pause, `+`/`-` adjusts time, `p` postpones breaks, etc.).
* **Background gradients and themes** – The app no longer relies on static image wallpapers.  Instead, two colour pickers let you define a bespoke linear gradient that spans the entire page.  A helper function subtly darkens the gradient when dark mode is enabled to maintain contrast and clarity.  Accent colours can still be selected via **theme presets** (Apple Blue, Green and Pink) or a custom colour picker; the accent colour automatically generates lighter and darker shades.  You can enable **dark mode** to invert the UI, and slide‑out panels maintain a frosted‑glass effect for a refined, macOS‑like appearance.
* **Audio playback with genres** – A dedicated audio panel (🎵 icon) now lets you choose a **genre** (e.g., Focus, Minecraft, Jazz Club, Country Club, Library, Café, Deep Work) and then select a track within that genre.  Previous, next and play/pause buttons make it easy to navigate the playlist.  You can adjust volume with a responsive slider whose knob enlarges when dragged, mute the audio entirely and scrub through the track with a progress bar.  The selected genre and track persist across sessions.
* **Task manager** – Tasks are managed in a dedicated panel.  Add tasks one at a time, mark them complete with checkboxes or delete them entirely using the 🗑️ button (which now prevents the panel from closing when clicked).  The current task appears below the timer and automatically advances after each completed work session or when you mark a task done.
